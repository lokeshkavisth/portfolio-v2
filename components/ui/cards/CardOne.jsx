  ...
  return (
    ...
    >
      ...
      <div className="space-y-2">
        ...
        <p>{description}</p>
{stack && <ul className="flex items-center flex-wrap gap-2">
  {stack.map((tech, index)=>(
    <li key={index} className="border border-black_02 py-1 px-2 rounded-md bg-black_03 hover:bg-black_02 hover:bg-opacity-50 transition-all">{tech}</li>
  ))}
</ul>}
      </div>
    </div>
